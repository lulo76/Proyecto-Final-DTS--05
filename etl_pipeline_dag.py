...
with DAG(dag_id="etl_2", catchup=False, default_args=default_args) as dag:

    with TaskGroup("create_data_base", tooltip="Crea base de datos") as create_data_base:
        create_db_ = create_db()
        create_db_

    # [START howto_task_group_section_2]
    with TaskGroup("etl", tooltip="crear etl") as etl:
    ...
    with TaskGroup("create_ER_Model", tooltip="Crea base de datos") as create_ER_Model:
        normalize_data_ = normalize_data()
        set_primary_key_ = set_primary_key()
        set_foreign_key_ = set_foreign_key()
        normalize_data_ >> set_primary_key_ >> set_foreign_key_


    create_data_base >> etl >> create_ER_Model